...
std::set<int> **get_tasks_pre_eff2_helper(bool *is_finished, int all_finished,
                                          int numTasks, int *numMethodsForTask, int **taskToMethods, int *numSubTasks, int **SubTasks,
                                          std::set<int> **&tasks_pre_eff)
{
       while ((finished_tasks < (*m).numTasks))
       {
              for (int parent = 0; parent < (*m).numTasks; parent++)
              {
                     // if the parent has not collected
                     bool collect = !(is_finished[parent]);
                     // check if children for all methods are done
                     for (int i = 0; i < (*m).numMethodsForTask[parent]; i++)
                     {
                            ...
                            for (int subtask_pos = 0; subtask_pos < (*m).numSubTasks[method]; subtask_pos++)
                            {
                                   ...
                                   collect = (!(is_finished[subtask])); // if edge not finished
                            }
                     }
...
std::set<int> **get_tasks_pre_eff2(Model *m, bool collect_statistics)
{
       ...
       if (collect_statistics)
       {
              ...
              file << "Recursive: " << ((recursive) ? "True " : "False ");
       }
       ...
       int finished_tasks = (*m).numActions;
       get_tasks_pre_eff2_helper(m, tasks_pre_eff, back_edges);

       // BACK RUN
       bool *is_finished //  bool is_finished[(*m).numTasks] = {false};
           int all_finished = 0;
       int numTasks;
       ...
       get_tasks_pre_eff2_helper(m, tasks_pre_eff);
