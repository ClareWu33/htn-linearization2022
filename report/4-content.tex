...
%\textbf{0.3} \textit{If Algorithm 1 didn't have to cycle break, if $a \in prec^{*}(t)$, then there exists an decomposition
%t' such that t' will add a} \newline
%\textit{Proof.} Assume a domain D linearized without cycle-breaking.
%Assume some compound task $t'$ and $a \in \AddS(t)$. We prove that there exists an decomposition of t'into actions $\langle a_0 ... a_n \rangle$ such 
%$a \in add(a_i)$ and $\forall k$ such that $a \in del(a_k)$,  $k < i$

%Base case: Assume some compound task $t$ decomposes into a non-empty set of actions A=$\{t', ...\}$,
%such that $a \in F, a \in add(t').$ This means $a \in \AddS(t)$. 
%If there exists $t'' \in A$ such that $a \in del(t'')$. By the algorithm rules
%\begin{itemize}			
%	\item Add $\{ (t, t') |  (a \in add^{*}(t) \land a \in prec^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ 
%	\item Add $\{ (t', t) |  (a \in add^{*}(t) \land a \in del^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ %
%	\item Add $\{ (t', t) |  (a \in del^{*}(t) \land a \in prec^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ 
%	\item Add $\{ (t, t') |  (a \in del^{*}(t) \land a \in add^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ 
%\end{itemize}
%We know that $t_1$ is ordered before $t_2$. Therefore, $a \in add(t')$ and $\forall t''$ such that $a \in del(t''))$,  $(t'', t') \in \prec$


%Assume for Inductive Hypothesis that if there exists an decomposition of $t$ into $\langle t_0 ... t_n \rangle$, then for each 
%$t_n$= $\langle t'_0 ... t'_n \rangle$, $a \in add(t'_i)$ and $\forall k$ such that $a \in del(t'_k)$,  $k < i$
%Step case: 
%Assume some compound task $t'$ decomposes into at least two actions: $\{t_1, t_2, ...\}$,
%such that $a \in F, a \in add(t_1), a \in del(t_2)$. This means $a \in \AddS(t)$. By the algorithm rules
%\begin{itemize}			
%	\item Add $\{ (t, t') |  (a \in add^{*}(t) \land a \in prec^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ 
%	\item Add $\{ (t', t) |  (a \in add^{*}(t) \land a \in del^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ %
%	\item Add $\{ (t', t) |  (a \in del^{*}(t) \land a \in prec^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ 
%	\item Add $\{ (t, t') |  (a \in del^{*}(t) \land a \in add^{*}(t') \land t, t' \in tasks(m) ) \}$ to $NS_m$ 
%\end{itemize}
%We know that $t_2$ is ordered before $t_1$. 
%This means that for $t'$, and $a \in \AddS(t)$, that there exists an execution of $t'$ that will add a.
...
Using this new domain, define the relaxed guaranteed and relaxed possible effects $\RelEffPlus$, $\RelEffMinus$,  $\RelPossEffPlus$, $ \RelPossEffMinus$ as before.
		...
		\For{e $\in$ BackEdges}{
			...
			\Else { 
				\Do {w > 1} {
					A, B = e \; 
					Path = Dijkstra(B, A) \;
					randEdge = RANDOM( \{e $\vert$ weight(e) = 0 $\land$ w $\in Path$ \} ) \;
					\uIf {randEdge exists} {
					 	E = E $\setminus$ randEdge
					} \Else {
						w := w - 1
